  ...
  return (
    <section id="projects" className="py-16 px-4 sm:px-6 md:px-12 lg:px-20 bg-white">
      ...
      <div className="text-center mb-10">
        ...
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-gray-700 mt-3 mb-2">
          Projets Réalisés
        </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {projects.map((project: Project, index: number) => {
          ...
          return (
            <div
              key={project.id}
              className="rounded-2xl border border-gray-200 overflow-hidden bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col"
            >
              ...
              <div className="p-5 sm:p-6 pt-4 border-t border-gray-100 flex flex-col flex-1">
                    ...
                    <ul className="mb-5 space-y-1 flex-1">
                      {features.map((feature, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs text-gray-700">
                          <span className={`${color.dot} mt-1 text-[8px]`}>●</span>
                          {feature}
                        </li>
                      ))}
                    </ul>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-auto">
                  ...
                  <a
                    ...
                    className="flex items-center justify-center py-2 rounded-lg text-xs font-semibold border border-gray-200 text-gray-700 bg-white hover:bg-gray-50 transition-colors"
                  >
                    Code source
                  </a>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`flex items-center justify-center py-2 rounded-lg text-xs font-semibold text-white hover:opacity-90 transition ${
                      color.btn.startsWith("from-") ? `bg-gradient-to-r ${color.btn}` : ""
                    }`}
